...
import { Status, statusLabels } from "@/lib/schemas/applications";
...
export default function MyApplicationsCard({item}: {item: MyApplications}){
    ...
    const deleteMutation = useMutation({
            ...
            mutationFn: async ()=>{
                const response = await api.applications({ id: item.id }).delete()
                if (response.error) {
       ...
       <div className="mx-auto md:mx-0 relative w-[310px] h-fit bg-gray-component border border-gray-border rounded-[16px] text-tBlack-main shadow-custom3">
            <div className="p-4  flex flex-col gap-4 ">
                ...
                <div>
                    ...
                    {item.vacancy && (<div className="flex gap-2 items-center">
                ...
                <div  className="flex gap-2">
                    <p className="text-[14px] text-tGray-sub">{item.vacancy ? "Отклик от: " : "Приглашение отправлено: "}</p>
                    <p className="text-[14px] text-tBlack-main">{item.createdAt.toLocaleDateString("ru-RU", { timeZone: "UTC" })}</p>
                </div>

                {item.status =="pending" && (
                    <Button variant={"outline"} onClick={()=>deleteMutation.mutate()}>Отозвать</Button>
                )}
            </div>
        </div>
    )
